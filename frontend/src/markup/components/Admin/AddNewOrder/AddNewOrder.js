...
function AddNewOrder() {
  ...
  const fetchData2 = async () => {
    try {
      ...
      if (!data2) {
        // Handle the case when data2 is undefined or null
        setVehicle_Error("Failed to fetch vehicle data. Please try again later.");
        setSpinner(!spin);
        return;
      }
      ...
      setVehicle_Error(error.response?.data?.error || "An error occurred while fetching vehicle data. Please try again later.");
      setSpinner(!spin);
    }
  };
