...
// get Customer Vehicle by customer id
async function getVehicleeById(hash) {
    try {
      const customer_hash = hash;
      ...
      if (rows.length < 1) {
        return []; // Return an empty array if no customer is found
      }
      ...
      console.error("Error in getVehicleeById:", error);
